...
function getCity(val)
{
	$.ajax({
	      ...
	      $.each(mainData1 , function(i , obj)
	  		{	
	    	  $('#city').append('<option value='+obj.cityId+' >'+obj.cityName+'</optipon>');		
	  		});
	      }
	      });     
}
...
-	$(".displayNone").css("display","block");
	 {
		...
		var j=1;
		$('#newTable tr').remove();
		$('#newTable').append('<tr  class="background-open-vacancies"><th>S.No.</th><th>Assessment Agency Id</th><th>Assessment Agency Name</th><th>Weblink</th><th>Status</th><th>Option</th></tr>')
		$.each(mainData1 , function(i , obj)
		{
			$('#newTable').append('<tr id="tableRow"><td>'+j++ +'</td><td>'+obj[1]+'</td><td>'+obj[2]+'</td><td>'+obj[3]+'</td><td>'+obj[4]+'</td><td><input type="hidden" id="maaId'+i+'" value="'+obj[0]+'" /><a href="#" onClick="editManageAssessmentAgency('+i+');">edit</a><input type="hidden" id="sid" value="'+obj[13]+'"><input type="hidden" id="sid" value="'+obj[14]+'"> <input type="hidden" id="sid" value="'+obj[15]+'"></td></tr>');	
		});
		}
		});
		$('#newTable').show();
...
function getStateUpdate(ss , dd , cc)
{
	$.ajax({
	      ...
	      $('#stateId').append('<option value="0" label="Select Stateeeeee" />');  
	      $.each(mainData2 , function(i , obj)
	  		{	
	    	  if(ss == obj.stateId){
	    		  $('#stateId').append('<option value='+obj.stateId+' label='+obj.stateName+' selected="true" />');	
	    	  }else{
	    		  $('#stateId').append('<option value='+obj.stateId+' label='+obj.stateName+' />');	
	    	  }	
	  		});
	      }
	      });
	...
	$( "#createDiv" ).hide();
	 {
		...
		$.each(mainData1 , function(i , obj)
		{
			document.getElementById("assessmentAgencyName").value = obj[2];
			document.getElementById("userId").value = obj[1];
			$("#assessmentAgencyName").attr("disabled", "disabled"); 
			$("#userId").attr("disabled", "disabled"); 
			$("#PAN").attr("disabled", "disabled");
			
			document.getElementById("PAN").value = obj[5];
			document.getElementById("websiteUrl").value = obj[3];
			document.getElementById("email").value = obj[6];
			document.getElementById("headOfficeDataAddress1").value = obj[7];
			document.getElementById("headOfficeDataAddress2").value = obj[8];
			document.getElementById("pin").value = obj[9];
			
			
			var s = obj[10]; var ss = obj[13];
			var d = obj[11]; var dd = obj[14];
			var c = obj[12]; var cc = obj[15];
			...
			if(obj[4]=="A"){
				...
				$('#status').append('<option value="0">Select Status</option><option value="A" selected="true">Active</option><option value="I">In-active</option>');
			}else{
				//alert('i');
				$('#status option').remove();
...
function updateData(){
	...
	var total = "userId="+userId+"&status="+status+"&websiteUrl="+websiteUrl+"&email="+email+
	"&headOfficeDataAddress1="+headOfficeDataAddress1+"&headOfficeDataAddress2="+headOfficeDataAddress1+
	"&pin="+pin+"&stateId="+stateId+"&district="+district+"&city="+city;
	$(".displayNone").css("display","block");
	$( "#updateDiv" ).hide();
	$( "#createDiv" ).show();
	$.ajax({
	...
	$('#status option').remove();
	$('#status').append('<option value="0" >Select Status</option><option value="A">Active</option><option value="I" >In-active</option>');
...
<cf:input path="pin" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')"  placeholder="PIN" class="form-control" maxlength="6" /></div>
                                    </div>
                                    <!-- right side -->
                                    <div class="col-md-6 col-xs-12">
...
<a href="#" onclick="updateData();" class="btn btn-default pull-right show-details-vacancy collapsed" 
...
<a href="#" onclick="searchData();" class="btn btn-default pull-right show-details-vacancy collapsed" 
